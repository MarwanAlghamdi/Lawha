...
Items 7, 8 and 12 of the four-phase batch. Of the other three, **9 is done, 10 is half done, and only 11 is untouched** — known issue 28 has the detail and is the authoritative one. This line said all three were outstanding, and kept saying it for a month after entry 28 had recorded otherwise **in the same session**, which is known issue 16 happening inside a single file: a summary and its own footnote disagreeing because only one of them was updated.

> Re-measured against the code on 2026-08-26 rather than settled by reading the two entries against each other, because that is the only way to tell which of two documents is the stale one. Item 9: there is no `POST /admin/users/:userId/password` — `admin.ts:294` mounts `/users/:userId/reset-code` instead (ADR 0021, migration `017_password_reset_codes.sql`). Item 10: `clients.ts:459` draws a named peer's initial and leaves guests the crewmate, and `cursorIdentity` appears nowhere in the tree, so the drawing half shipped and the three-way setting did not. Item 11: no file under `excalidraw-app/lawha/` handles `contextmenu` at all.
...
3. ~~**Presence names still lag for peers who have not moved.**~~ Closed by the identity event (ADR 0006). Excalidraw only transmits a username inside pointer and idle payloads, so a peer who had not moved showed as "Joining…". The server now announces every member's name on join, before anyone has moved a pointer — including link guests, who get a stable placeholder rather than a blank. Pinned by `identity.test.ts` ("names a signed-in peer from the account, not from the socket", "announces identities to the room when someone joins") and, on the client, by `lawhaIdentityPinning.test.tsx`, which is where the merge direction that could have undone it now lives.
   ...
   > **And then both of those files were deleted — see entry 31.** `identity.test.ts` and `lawhaIdentityPinning.test.tsx` went with `59930dbf` on 2026-08-14. The fix is still in the code and the behaviour is still correct; what is gone is the thing that would notice if it stopped being. Read the paragraph above as history: it records which citation was right, in a file where neither citation resolves any more.
...
20. **Invariant 10's count is stale: `packages/` diverges in seventy-one paths, not fourteen.** Measured, not recalled — `git diff --name-status $(git merge-base upstream/master main)..main -- packages/` reports 22 added and 49 edited, and `git diff --stat` on the same range reports 11,120 insertions against 117 deletions (re-run 2026-08-26; unchanged from the 2026-08-25 measurement below). The invariant has never quite matched its own list and neither had this entry's headline, which said "thirteen, not four" while the body four paragraphs down already said seventy-one. Both are fixed now; the table below is the recorded subset, not the whole 71. What is actually there:
    ...
    | `excalidraw/components/TTDDialog/TTDDialog.tsx` | ADR 0028. Was 0, now 23 — the converter loader indirection | | `excalidraw/components/TTDDialog/mermaidLib.ts` | ADR 0028. **New file**, so free at merge time | | `element/src/transform.ts` | ADR 0028. Was 0, now 205 — `table`/`tensor`/`code` in the element-skeleton API, and an arrow can bind to them |

    **Re-measured 2026-08-25 after the ADR 0029/0030 batch: `packages/` diverges in 71 tracked paths — 22 added, 49 edited — 11,120 insertions** (was 63 / 9,444 before ADR 0027, then 71 / 10,456 once the mermaid batch was committed). ADR 0030 added **no new paths at all**: every tensor change lands in a file ADR 0026 already touched, and ADR 0029 is entirely outside `packages/`. The `mermaid*` and `TTDDialog/` rows are the ones that matter, because every one of those files was at _exactly zero_ before and the directory was pristine upstream. `App.tsx` grew by two lines — an import and one changed expression. The converter itself is ~2,750 lines in `excalidraw-app/lawha/mermaid/`, which upstream does not have and never will, and therefore costs nothing at merge.
...
22. ~~**The restore half of backup/restore has never been exercised.**~~ **It was worse than unexercised: the documented procedure destroyed the database.** Its step 2 was `docker volume rm excalidraw_lawha-data`, and somebody followed it. Replaced wholesale in §4.13 — the named volume is gone, the `docker volume` step is gone, and both halves are now scripts with tests (`lawha-server/scripts/backup.mjs`, `restore.mjs`, 127 tests across four `scripts/*.test.mjs` files — measured 2026-08-26, and eighteen more run where `age` is on PATH). The ownership hazard this entry used to name went with the volume: the host user and the container's `node` are both uid 1000, so there is no root-owned extraction any more.
...
23. **Two of §4.12's five fixes are unpinned.** The three that could change what crosses the wire or comes out of the database each got a test next to the line that refuses — `avatar.test.ts`, `lawhaIdentityPinning.test.tsx`, `folders.test.ts`. The two low-severity ones did not. The folder rail's missing React key has no runtime effect and was a development-console warning only, which is a fair reason to skip it; the dashboard selection bar's `isBulkRunning` lock has no assertion that a running bulk delete disables its own button, which is not. `LawhaHomeSelection.test.tsx` already renders that bar and is where it belongs. Left open rather than waved away, because "it is obviously right" is the argument that kept `canEdit` unenforced for months (invariant 21).
    ...
    > **The headline is now wrong in the direction that matters: all five are unpinned.** All four files this entry names — `avatar.test.ts`, `lawhaIdentityPinning.test.tsx`, `folders.test.ts` and the `LawhaHomeSelection.test.tsx` it proposed writing into — were removed by `59930dbf` (entry 31). So the three fixes it called safely pinned are in exactly the same state as the two it called unpinned, and the distinction the entry is built on no longer exists. Left standing rather than rewritten, because **which three were worth pinning is still the useful part** and would be the shape of the work if these are ever re-pinned: the ones that change what crosses the wire or comes out of the database.
...
24. **One test in `collabBoardReopen.test.tsx` passes against the unfixed code.** "does not take a peer's empty `SCENE_INIT` for the board's scene" mounts a _single_ board, and on the first board of a page session `App.initializeScene` reads storage itself, so the old code shows the stored rectangle too. The other two tests in that describe block — the ones that open a second board — fail hard without the fix, so the bug is pinned; this one is a forward-looking assertion about the empty-INIT rule, not a witness to the defect. Either give it the two-board shape its siblings have, or leave it and know what it is. Recorded because this repository has shipped a regression test that passed both ways before, and the way that happens is nobody writing down which ones do.
    ...
    > **Moot, and worth keeping for the reason it was written.** `collabBoardReopen.test.tsx` was removed by `59930dbf` (entry 31), so there is no test to reshape and no suite to reshape it in — the two siblings that _did_ fail hard without the fix are gone with it, which means the empty-`SCENE_INIT` rule now has no witness at all rather than one weak one. The entry stays because its subject was never this file: it is that a test passing both ways is invisible unless somebody writes down that it does.
...
27. **`LawhaAuth.test.tsx`'s avatar test is flaky, and it is not the escrow work's fault — measured, not assumed.** "PUTs the raw bytes and re-reads the account for the new id" failed **3 runs out of 3 on a stashed, unmodified checkout** while this was being diagnosed, so it is pre-existing. Two separate causes were found and only one is fixed:
    ...
    > **The flake is gone; the bug it was flaking on is not.** `LawhaAuth.test.tsx` was removed by `59930dbf` (entry 31), which closes the first half of this entry by deletion — there is no 178th file, no worker pool to starve, no deadline to raise. The second half is untouched and is the half that mattered: **something in the avatar re-read path intermittently does not re-render**, in `LawhaAccountPanel`, at roughly 1 in 4 before the file went. Nobody has chased it, and now nothing will report it — it will present as a user saying their new picture did not take. This is the clearest single example of what entry 31 costs: the suite that was noisy was also the only thing looking.
...
31. **Sixty citations in this repository named a test file that no longer exists.** Every one pointed into `lawha-server/tests/` or `excalidraw-app/tests/`, neither of which is a directory any more. This is the answer to "why does the roadmap keep naming test files that are not here", and the answer is one commit rather than thirty separate mistakes.

    `59930dbf` (2026-08-14, `chore!: remove Lawha's own test suites`) deleted **106 files and 35,055 lines** — 47 under `lawha-server/tests/`, 59 under `excalidraw-app/tests/`, of which 100 were test files and six were helpers and a snapshot — carrying **1,314 tests**. It was **requested deliberately and recorded plainly**; its message is worth reading before touching any of this, because it enumerates what went and says in as many words that the fixes stay in the code and only the pins go. Nothing here second-guesses that decision. What was never done is the sweep afterwards.

    **The files are recoverable, and that is not a figure of speech:** they were committed before being removed, so `git show 59930dbf^:lawha-server/tests/integration/folders.test.ts` prints the file. Any citation below can be read by anyone who wants it.

    **Measured 2026-08-26, not recalled: 60 lines across 28 files** name one of the 97 distinct deleted test basenames. Nineteen are in this file. Sixteen of the twenty-eight are code or config rather than prose, and those are the ones that do damage, because a comment saying "`X.test.ts` pins this" reads as a live guarantee to the next person deciding whether a line is safe to change. The commit itself made exactly this argument about one comment in `authApi.ts` — "a comment asserting a security property is enforced when it is not is worse than no comment" — fixed that one, and left the rest.

    **What actually lost its only witness.** This is the part worth acting on; every row was checked against the current tree rather than inferred from the deletion.

    | Property | Was held by | State now |
    | --- | --- | --- |
    | `/backups:ro` on the admin download, and no writable second mount beside it | `backupCoverage.test.ts` (ADR 0017) | **Re-pinned 2026-08-26** in `deploymentConfig.test.mjs`, structurally rather than by service name — see below |
    | `COUNTED_TABLES` identical in `src/lib/backupVerify.ts` and `scripts/backup.mjs` | `backupVerifyParity.test.ts` | In sync, unguarded — and there are **four** copies now, not two (`backup.mjs`, `restore.mjs`, `encrypt-db.mjs`, `backupVerify.ts`) |
    | `OVERDUE_GRACE_MS` (1h, `backupArchive.ts:124`) = `OVERDUE_GRACE_SECONDS` (3600, `docker/lawha-backup.sh:155`) | `backupCoverage.test.ts` | In sync, unguarded. Drift shows as a card reading "Running normally" beside an unhealthy container — the exact bug §4.13 fixed |
    | `COLLABORATOR_PALETTE.length` = the index bound the server accepts | `account.test.ts` | Unguarded. This one already failed once: the two sat at 12 and 5 for a whole phase and seven swatches answered 400 |
    | A peer's empty `SCENE_INIT` never becomes the board's scene | `collabBoardReopen.test.tsx` | Unguarded (entry 24) |
    | The avatar re-read re-renders | `LawhaAuth.test.tsx` | Unguarded, **and known broken** at ~1 in 4 (entry 27) |
    | `cipher=sqlcipher` is set **before** `PRAGMA key` in `src/db/index.ts` | `dbEncryption.test.ts` | Unguarded, and the worst of the seven: key-first silently writes a **chacha20** database, so every existing encrypted database stops opening and the operator is told the key is wrong |

    **That last row was measured by breaking it, and the measurement caught a mistake of my own.** Swapping the two pragmas and running both gates gives **125/125** from `node --test scripts/*.test.mjs` (the count before the two assertions added below; 127 now, and neither of them touches this) and **82/82** from the seven vitest files under `lawha-server/src/` — the branch is entered only when a key is configured and nothing in either gate configures one. The comment above those lines had claimed, since **2026-08-25**, that `scripts/encrypt-db.test.mjs` would fail on a reorder. It would not: that file pins how the key is _escaped_ and an ordering _inside `encrypt-db.mjs`_, not these two lines. **That claim was written in the previous commit while fixing exactly this class of error** — the first version of the comment had named the deleted `dbEncryption.test.ts`, and re-pointing it at a surviving file that looked close enough replaced a dangling citation with a false one. A citation that resolves is not thereby a citation that covers the line, and the second kind is worse, because it survives being checked.

    **One citation was false before the deletion, too — and re-pointing it would have preserved the wrong half.** `docker/nginx.conf` said a bad `log_format` name was caught by "`nginx -t` in `deploymentConfig.test.ts`". No test in this repository had ever run `nginx -t`: the `.ts` file parsed the config as text, and so did the `.mjs` that replaced it. The only real runs were by hand, in §4.20. **Now it is true**, which took writing the assertion rather than fixing the sentence.

    **Two of the seven were closed the same day, and both by assertion rather than by prose.** `lawha-server/scripts/deploymentConfig.test.mjs` goes from fifteen to **seventeen**:

    - **"the backup archive is read-only in every service that can be reached"** — stated structurally, because naming `lawha-server` would have pinned today's topology instead of the rule. Whoever declares `ports:` or `expose:` must mount the archive `:ro`; exactly one service may mount it writable; that one must declare neither; and both must resolve to the same host directory, since a typo there reads as "backups are broken" on a stack taking them perfectly well.
    - **"the nginx config actually parses"** — a real `nginx -t` against `nginx:alpine`. It **skips** when docker or the image is absent and never pulls, so a green run proves nothing on a machine without either. That is the `age` shape from `backup.test.mjs`, and it means this is a development-machine guard by construction: the runtime image has no docker.

    **Eight mutations, all eight caught** — dropping the `:ro`, making the writer read-only so nothing can write backups, splitting the host paths, giving the writer an `expose:`, giving the published service a writable mount, naming an undefined `log_format`, removing a semicolon, and reintroducing a `${...}` placeholder. The eighth is the one worth keeping: the test mounts the file verbatim because compose leaves no placeholders in it, so it asserts that this stays true rather than silently testing a file the deployment does not use.

    **Five remain unwitnessed**, and the pragma order is the one to fix next — it is the only one whose failure destroys data rather than degrading a feature.

    **What still runs, so the gap is measured from something real.** `yarn test:app` reports **131 files / 2,050 passing** (2,098 collected, 47 skipped, 1 todo), and **exactly eight of those files sit outside `packages/`** — `excalidraw-app/lawha/mermaid/index.test.ts` and seven under `lawha-server/src/`, every one of them added _after_ 2026-08-14 for ADRs 0026 through 0031. The other 123 are Excalidraw's own suite plus the few Lawha files that live inside `packages/tests/` by invariant 10. `yarn test:server` is four `scripts/*.test.mjs` files, **127 tests** (125 before the two added below), plus eighteen more where `age` is on PATH. `e2e/` is untouched and CI has never run it (known issue 30).

    **What this sweep changed, so the number above is readable later.** The sixty were counted on 2026-08-26 _before_ any edit. The sixteen code and config sites were rewritten the same day: each keeps the reasoning and now says the pin is gone, and each names `59930dbf`, so `git grep 59930dbf` is the index. The prose sites in `docs/adr/` were left alone deliberately — an ADR records what was true when it was decided, and rewriting one to match today destroys the thing it exists for. Four roadmap entries whose _claim_ depended on a deleted file (3, 23, 24, 27) carry a note instead.

    **The rule this leaves behind.** A citation is a claim, and claims rot faster than the code they describe — known issue 16, one level down. If a comment names a test, the name must resolve; if the test is gone and the property still matters, say so in the comment rather than leaving the name. The comments touched on 2026-08-26 do exactly that: they keep the reasoning, which is the valuable half, and stop asserting a guard that is not there.
...
The GitNexus index was corrupted four times with `FTS index 'file_fts' is inconsistent`. **The cause is the WAL auto-checkpoint threshold, not concurrency** — GitNexus said so itself the fourth time: _"LadybugDB failed while rotating/removing WAL checkpoint files. This can happen when auto-checkpoint runs at the default threshold (~16MB)."_ At ~12,900 nodes this index checkpoints often enough to hit it, a failed rotation leaves a quarantined `lbug.wal.missing-shadow.*` sidecar, and the _next_ run reports the FTS inconsistency. The script exports `GITNEXUS_WAL_CHECKPOINT_THRESHOLD=67108864`, the value the tool's own recovery hint suggests. The lock is still there — two `analyze` runs over one index is a bad idea regardless — but it was never the fix, and three rebuilds were spent chasing the wrong story.
...
corepack yarn test:server              # `node --test scripts/*.test.mjs` ALONE — 4 files, 127 tests.
