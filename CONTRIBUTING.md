...
`test:server` is a separate runner — `node --test scripts/*.test.mjs`, 4 files and 127 tests over backup, restore, encrypt-db and the deployment-config pins. Backup and restore are covered because a silent backup failure has cost this project real data.
